...
def generar_diagnostico(request, evaluación_id):
    ...
    try:
        evaluación = Evaluación.objects.get(
            id=evaluación_id, psychologist_id=str(request.user.id)
        )
    ...
    diagnóstico, created = Diagnóstico.objects.update_or_create(
        ...
        defaults={
            ...
            "actividades_estimulación": diagnóstico_data.get(
                "actividades_estimulación", []
            ),
        },
    )
...
def obtener_editar_diagnostico(request, evaluación_id):
    ...
    try:
        diagnóstico = Diagnóstico.objects.select_related("evaluación").get(
            evaluación_id=evaluación_id,
            evaluación__psychologist_id=str(request.user.id),
        )
    except Diagnóstico.DoesNotExist:
        return Response(
            {"error": "Diagnóstico no encontrado"}, status=status.HTTP_404_NOT_FOUND
        )
