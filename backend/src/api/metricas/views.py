...
def registrar_métricas(request):
    ...
    if not fecha:
        return Response(
            {"error": "fecha es requerida"}, status=drf_status.HTTP_400_BAD_REQUEST
        )

    parsed_fecha = _parse_iso_date(fecha, "fecha")
    if parsed_fecha is None:
        return Response(
            {"error": "fecha inválida"}, status=drf_status.HTTP_400_BAD_REQUEST
        )

    field = TIPO_TO_FIELD.get(tipo)
    if field is None:
        return Response(
            {"error": f"tipo inválido (esperado uno de {list(TIPO_TO_FIELD)})"},
            status=drf_status.HTTP_400_BAD_REQUEST,
        )
...
def obtener_métricas_tesis(request):
    ...
    total_completadas = evaluaciones.filter(
        estado__in=[
    ...
    tasa_automatizacion = round(
        (auto_validados / total_items * 100) if total_items > 0 else 0, 1
    )

    evals_completadas = evaluaciones.filter(
        completed_at__isnull=False, started_at__isnull=False
    )
    durations = [
        (e.completed_at - e.started_at).total_seconds() / 60.0
        for e in evals_completadas
